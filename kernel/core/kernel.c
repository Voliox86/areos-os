...
static const man_page_t man_pages[] = {
    ...
    {"date",     "Print the current date and time from the CMOS real-time clock. With a `+FORMAT` argument, format it strftime-style: %Y %y %m %d %e %H %I %M %S %p (year/month/day/hour/min/sec), %A/%a (weekday name), %w/%u (weekday number: %w 0-6 Sun=0, %u 1-7 Mon=1), %B/%b (month name), %j (day of year), %% — e.g. `date +%A` or `date +%u`."},
