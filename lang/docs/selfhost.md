...
N v0.25 (own fields inside own structs, spec §4.6) reached `check.n`
the same way every earlier rule did: `ncc` first, then the mirror,
held to the reference's own words. The containment loop now lets an
**own** container hold own fields and still refuses a plain struct
(the v0.17 text, unchanged); a struct literal's own-typed field
initializer is a move point; the move point refuses a field expression
of own type ("cannot move field 'file' out of own value 'l' — consume
'l' as a whole"); the auto-drop simulation asks `hasdrop` — a
destructor, or an own container whose every own field can drop — and
at every function exit a held container whose own field cannot drop is
reported before the leak scan. The corpus grew to **seventy-five
rows**, the four new ones generated by running ncc as always, and
[`ownnest.n`](../examples/ownnest.n) joined the silent set and the
parser differential.

A polish pass followed, mirrored in check.n and in gen.n's checker
half: the field-move refusal spells the place as written and names the
binding to consume — `cannot move field 'a' out of own value 'o.p' —
consume 'o' as a whole` (`mplace` walks the field chain down to its
root, as ncc's `expr_place` does; a place that is not a chain stays
"an own value"), and an empty body reports a held container's leak at
its opening brace instead of line 0 (the block node's own line, which
...
itself a destructor-less container through *its* fields (the value path
`name.field.field` is a small span stack), a held container's fields at
every function exit — the braced `__ret` form chosen when a held
container waits on them, a returning tail draining them exactly once —
and a live container without a destructor of its own ending through its
fields at scope end. The emit walk also moves a binding into the
struct literal that takes it, so it is never auto-dropped behind the
literal's back. [`ownnest.n`](../examples/ownnest.n) is in every
differential now, and stage [8d+] holds the exit paths it does not
walk: a returning tail, an early `return` from a function holding a
nested container, a void return, a moved-in binding. The `ngen`
package was regenerated from the new `gen.n`.
...
### The corpus grows — function types (v0.24)

When ncc learned function types, five refusals came with them —
arity and argument types through a fn value, a non-function called
through, a signature mismatch, a syscall named as a value, and a
bare function bound where no such fn type is declared — and the
corpus grew to **seventy-one rows**, every one generated by running
ncc, as before. check.n met them by extending the intern trick it
already lives on: ncc prints a function type in its N spelling,
`fn(A, B) -> R`, so check.n makes that spelling the type's NAME —
written to the buffer tail the first time a signature is parsed or a
function is named as a value — and a fn type is then an ordinary
`T` with ptrs 0 and a name span. Nothing downstream had to learn
anything: `span_eq` gives structural identity for free (two
signatures are the same type exactly when their spellings match byte
for byte, which is ncc's interned-name identity), `tcompat` refuses
the mismatch, and `ty_put` prints the diagnostic form without
knowing it is one. A small table keeps each signature's shape for
calls through values — a parameter, a local, or a struct field,
checked exactly like a direct call, in the same words — and counts
how many signatures the PARSE declared: ncc emits a typedef for
exactly those, so a function value of any other signature cannot be
bound, and check.n refuses it with ncc's sentence. `fntype.n` joined
the silent set: **71 rows byte-exact, 25 examples and all four
selfhost sources silent.** The probes beyond the corpus delivered
the differential's second catch of the REFERENCE: a signature nested
in a signature — `fn(fn(i64) -> i64, i64) -> i64` — came out of ncc's
diagnostics spelled by its typedef name, `fn(__nyx_fn0, i64) -> i64`,
an internal name no N program ever wrote; check.n, spelling from the
tail, had it right. ncc now spells a nested signature in full, and
the two agree. The generator is the last module to catch up.
...
### ngen ships as a package

`xbm install ngen` puts the self-hosted compiler on any NyxOS. The
package respects xbm's contract — one C source, built with the in-OS
`cc` — by shipping exactly what the pipeline produces: `ngen.c` is
ncc's emitted C for `gen.n` with `user/nyxrt.c` appended (one
self-contained translation unit; its first line, `Generated by ncc
from gen.n`, is its own provenance), beside a local `nyxrt.h` so the
quoted include resolves package-first. Nothing in it is hand-written:
the suite's [1c] guard regenerates the whole file from `gen.n` on
every run and byte-compares, so the package can never drift from the
source it claims to be. The install story is the milestone in one
command: a compiler written in N, translated by the C-hosted ncc,
