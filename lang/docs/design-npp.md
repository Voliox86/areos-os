...
closure written by hand. The `FnOnce(A) -> R` slot type that lowers to
it is M6.4c6, next.*
...
| M6.4c6 | 🔜 **owning closures — the `FnOnce(A) -> R` slot type**, on N v0.26 ("Behind a raw pointer": a pointer to an own type as a local born from a cast; store moves in, peek moves nothing, take gives a fresh owner — [`ownbox.n`](../examples/ownbox.n)). The lowering is the hand-written closure in ownbox.n: one `#[drop(__fo_drop_<sig>)] own struct __FnOnce_<sig> { env: addr, call: fn(addr, A…) -> R, fin: fn(addr) }` per signature, the lambda's environment an `own struct __E_N` stored through `__mk_E_N` (its own captures moved in), a per-lambda finaliser `__fin_E_N(_env: addr)` that takes the environment out and lets it drop, `__call_once_<sig>(f, x)` running `f.call(f.env, x)` and then the drop, every `f(x)` on a `FnOnce` name rewritten to it. N then gives must-consume, one owner, auto-drop of a closure never called, and "use of 'f' after move" on a second call — an own capture may finally be passed where a closure is expected | ownbox.n runs the shape by hand under every differential (suite [6f]); the lowering is next |
