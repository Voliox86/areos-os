...
*Shipped as M6.4a–c3c (§6.4): the capture struct lives on the bump heap
and the closure carries its address, so a closure outlives its frame.
An `own` capture is the part still open, and it is an N question before
it is an nppc one. The environment would have to be an `own` struct
holding an `own` field — N v0.25's rule, designed in
[spec-n.md §4.6](spec-n.md) ("Own fields inside own structs") — and it
would have to be reachable from the closure value, which today means
through a pointer, and N refuses pointers to own values so that an owner
has exactly one binding. Two shapes are on the table, to be decided with
the c4 rung once v0.25 has landed in `ncc` and the toolbox: (i) an own
environment passed **by value** — the closure value itself becomes an
own struct, consumed by its one call, a call-once closure the checker
can track with the rules it has; (ii) an own heap box in N — a single
pointer-like own type the language knows how to drop — which is a
bigger N rung of its own. Neither is chosen here.*
...
| M6.4c4 | `own` captures: the environment inherits must-consume (design §6.2) — **waits on N v0.25.** N v0.24 lets no `own` value nest in a struct or sit behind a pointer (spec §4.6), and a closure's environment is exactly that — a heap struct reached through `*__E_N`. The nesting half is **in `ncc` since v0.25**: spec §4.6 "Own fields inside own structs" (an `own struct` may hold `own` fields; construction moves; a field never moves out alone; a held container's end drops its fields in reverse declaration order, its own `#[drop]` included; moving the container moves everything), [`ownnest.n`](../examples/ownnest.n) showing the drop order; the toolbox followed — `check.n` mirrors the diagnostics (a 75-row corpus), `gen.n` emits the field drops — and `ownnest.n` sits in every differential. The pointer half stays open (§6.2: an own environment by value — a call-once closure — or an own heap box in N). The `win.on(Event.Click, fn(e) …)` handler sugar over [nwin.n](../examples/nwin.n)'s polled event loop is library design on top of closures rather than lowering, and follows a real N++ window module | an event-handler example; escape/negative tests |
