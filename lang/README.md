...
four of them, [closure.npp](examples/closure.npp),
[capture.npp](examples/capture.npp), [fnonce.npp](examples/fnonce.npp)
and [gfnonce.npp](examples/gfnonce.npp), has been compiled and run
**inside NyxOS** by the in-OS `ncc`. With the lowered `.n` files and
`nyxrt.{c,h}` on the `/mnt` disk,
...
200 sees 2` / `d = 201`), and the other three match their host traces
line for line — lambdas lifted to top-level functions, environments on
the sbrk heap, owning closures with their finalisers, generic templates
instantiated per type: all of it plain N by the time the OS sees it,
and plain C99 by the time TinyCC does. The transcript is recorded in
