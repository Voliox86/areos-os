...
- **Preemptive scheduler** — DONE. Kernel threads (v5.7.6, PIT via I/O APIC pin 2
  ...
  safely. `jobs`/`wait`/wait-all (v5.7.11) round out shell job control. Remaining
  follow-ups:
  1. **Priority / IO waits.** A priority/nice scheme so the compositor beats
     background jobs (weighted round-robin: give a proc N ticks per turn). An
     IO/event wait queue (keyboard, net) is trickier — the compositor must still
     wake periodically for the clock/mouse/net, so it can't purely block on input.
     Still busy-ish: mtdemo/spin `hlt`-park after finishing rather than exiting.
