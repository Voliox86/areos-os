...
- **Userspace polish** (ring-3 works as of v5.5.0; exec reaps on exit + file
  ...
  - Jobs: `exec <file>` = foreground (blocks via `kwait`), `spawn <file>` =
    background (v5.7.8). Still missing: `wait`-any / multiple children, and `kill`
    on a *running* scheduled proc (mark it a zombie so the scheduler drops it).
- **Preemptive scheduler** — DONE. Kernel threads (v5.7.6, PIT via I/O APIC pin 2
  ...
  background. Legacy setjmp/longjmp blocking-exec path removed (v5.7.9) — one
  ring-3 launch path (scheduler `iretq`) remains. Remaining follow-ups:
  1. **Blocked states + wait queues (general).** `kwait` proves the pattern; extend
     it: `sleep()` still busy-waits on `tick_count`, and mtdemo/spin `hlt`-park.
     Timer/IO wait queues + a priority/nice scheme would free the CPU and keep the
     compositor responsive under load. Also `kill` on a *running* scheduled proc
     (mark it a zombie so the scheduler drops it) and `wait`-any / multiple children.
  2. **Heap/VFS re-entrancy.** `kmalloc`/`kfree` are now `preempt_disable`-guarded;
     the VFS and other shared kernel state still assume a single thread. Audit
     before letting spawned procs hammer the FS concurrently.
