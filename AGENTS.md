...
- **Preemptive scheduler** — DONE. Kernel threads (v5.7.6, PIT via I/O APIC pin 2
  ...
  2. **Per-process FS state.** Fds are now per-process (v5.7.14: `ufd` table in
     `process_t`, isolated + closed on reap). Still global: `current_dir` (CWD) —
     move it into `process_t` too when there's a reason (multiple shells, or user
     procs that use relative paths — today they use absolute, so it's low value and
     `resolve_path` is core). Also: the FS lock is coarse (whole EXT2 op under
     `preempt_disable`) — fine now, but a buffer-per-op would let FS I/O interleave.
...
- Network: DHCP/DNS/TCP/HTTP all work (v5.7.3–v5.7.4); loopback + ICMP echo replies
  ...
  loopback TCP with a `tcploop` self-test **done** (v5.7.19). Possible follow-ups:
  TCP send-side segmentation for large request bodies, NIC-side `listen` (thread the
  received dst IP through so a child sources from the right interface), and DNS
  ...
  `ping 127.0.0.1`) or a pcap dump (`qemu -object filter-dump,id=f0,netdev=n0,file=x.pcap`).
